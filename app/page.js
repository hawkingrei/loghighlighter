...
export default function Home() {
  ...
  const [useAdvancedFlags, setUseAdvancedFlags] = useState(false);
  ...
  useEffect(() => {
    ...
    setUseAdvancedFlags(stored.settings.useAdvancedFlags);
      ...
      <header className="hero">
        <div>
          ...
          <p className="subtitle">
            Paste logs or load from a URL, then highlight with customizable regex rules. Rules are
        ...
        <div className="settings-overlay" role="dialog" aria-modal="true">
          <div className="panel settings-panel">
            ...
            <label className="toggle">
              <input
                type="checkbox"
                checked={useAdvancedFlags}
                onChange={(event) => setUseAdvancedFlags(event.target.checked)}
              />
              <span>Advanced per-rule flags</span>
            </label>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Name</th>
                    <th>Pattern</th>
                    {useAdvancedFlags ? <th>Flags</th> : null}
                    <th>Color</th>
                    <th>Note</th>
                    <th>Action</th>
                  </tr>
                      ...
                      <tr key={rule.id}>
                        ...
                        <td>
                          <button className="ghost" onClick={() => handleRemoveRule(rule)}>
                            {rule.source === "default" ? "Disable" : "Remove"}
                          </button>
                        </td>
                      </tr>
                    );
...
function loadStored() {
  if (typeof window === "undefined") {
    return {
      overrides: {},
      custom: [],
      settings: { globalFlags: "gi", useAdvancedFlags: false, lineTextColor: "#BE6DAB" },
    };
  }
  ...
  try {
    ...
    return {
      ...
      settings: {
        ...
        useAdvancedFlags: parsed?.settings?.useAdvancedFlags ?? false,
...
function getEffectiveFlags(rule, globalFlags, useAdvancedFlags) {
  const ruleFlags = useAdvancedFlags ? rule.flags : "";
  return mergeFlags(globalFlags, ruleFlags);
}
...
function buildFlagOptions(globalFlags) {
  return [
    { key: "g", label: "Match all occurrences", locked: true, checked: true },
    { key: "i", label: "Ignore case", locked: false, checked: globalFlags.includes("i") },
    { key: "m", label: "Multiline anchors", locked: false, checked: globalFlags.includes("m") },
    { key: "s", label: "Dot matches newline", locked: false, checked: globalFlags.includes("s") },
    { key: "u", label: "Unicode mode", locked: false, checked: globalFlags.includes("u") },
    { key: "y", label: "Sticky matching", locked: false, checked: globalFlags.includes("y") },
  ];
}
